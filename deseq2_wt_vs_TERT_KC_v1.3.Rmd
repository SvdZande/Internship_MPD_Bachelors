...
p_marker <- ggplot(find_set_KC, aes(x = gene, y = log10(TPM+1))) +
  geom_bar(
    aes(fill = condition),
...
barplot(ego_high_log, showCategory = 15, title = "Barplot for log2 TERT-high genes")
barplot(ego_low_log, showCategory = 15, title = "Barplot for log2 TERT-low genes")
...
ggplot(find_set_e2f1_all, aes(x = gene, y = log10(TPM+1))) +
  geom_bar(
    aes(fill = condition),
    stat = "identity" , position = position_dodge(0.8),
    width = 0.7) + geom_signif(y_position = c(2.3,0.9,2.5,0.9,1.0,1.2,0.6,1.4,1.3,1.3,1.5), xmin = c(0.6,1.6,2.6,3.6,4.6,5.6,6.6,7.6,8.6,9.6,10.6), 
                               xmax = c(1.4,2.4,3.4,4.4,5.4,6.4,7.4,8.4,9.4,10.4,11.4), annotation = c("*","*","*","*","*","*","*","*","*","*","*"),
                               tip_length = 0.02)
