...
HTlsc <- Heatmap(z_logfilter, name="z-score", col = f4,  row_names_gp = gpar(fontsize = 5)) + rowAnnotation(mark = anno_lsc)
...
find_LSC_row <- which(rownames(z_logfilter)%in% find_LSC, arr.ind = TRUE)
anno_lsc = anno_mark(at = find_LSC_row, labels = find_LSC , which = "row" , labels_gp = gpar(fontsize = 18))
HTlsc <- Heatmap(z_logfilter, name="z-score", show_heatmap_legend = FALSE, col = f4, column_names_gp = gpar(fontsize = 20),  heatmap_legend_param = list(labels_gp = gpar(fontsize = 20), title_gp = gpar(fontsize = 24))) + rowAnnotation(mark = anno_lsc)
draw(HTlsc)
...
p_cell_LSC <- ggplot(find_set_cell, aes(x = gene, y = log10(TPM+1))) +
  geom_bar(
    aes(fill = condition),
    stat = "identity" , position = position_dodge(0.8),
    width = 0.7) + geom_signif(y_position = c(2.1, 3.4), xmin = c(0.6,8.6), 
...
find_cell_row <- which(rownames(z_logfilter)%in% cellcycle, arr.ind = TRUE)
anno_cell = anno_mark(at = find_cell_row, labels = cellcycle , which = "row")
HTcell_LSC <- Heatmap(z_logfilter, name="z-score", column_title = "Cell cycle (LSC)", col = f4,  row_names_gp = gpar(fontsize = 5)) + rowAnnotation(mark = anno_cell)
...
#Plot the result
high_log2_GO_LSC <- barplot(ego_high_log, order = T, showCategory = 15, title = "Differentially upregulated genes in TERT")
low_log2_GO_LSC <- barplot(ego_low_log, order = T,showCategory = 15, title = "Differentially downregulated genes in TERT")
high_log2_GO_LSC
low_log2_GO_LSC
...
HTcellfork_LSC <- Heatmap(z_logfilter, name="z-score",  
                          col = f4,  show_heatmap_legend = FALSE, column_names_gp = gpar(fontsize = 20),  heatmap_legend_param = list(labels_gp = gpar(fontsize = 20), title_gp = gpar(fontsize = 24))) + rowAnnotation(mark = anno_cellfork)
draw(HTcellfork_LSC)
...
PCA_pos_LSC <- barplot(ego_PC1_pos, order = T, showCategory = 15,title = "Genes giving positive weight to PC1")
PCA_neg_LSC <- barplot(ego_PC1_neg, order = T, showCategory = 15, title = "Genes giving negative weight to PC1")
...
splicing_genes <- read.csv("/scratch/szande_jarts/diff_gene_expression_wt_vs_TERT/20220627/diff_splicing_genes.v2.tsv", header = TRUE, sep= "\t") #old list: 20220525
...
splice_pos_GO <- barplot(ego_splice_pos, order = T, showCategory = 15, title = "Differentially spliced genes (high)")
splice_neg_GO <- barplot(ego_splice_neg, order = T, showCategory = 15, title = "Differentially spliced genes (low)")
...
splice_diff_pos_GO <- barplot(ego_diff_pos, order = T, showCategory = 15)
splice_diff_neg_GO <- barplot(ego_diff_neg, order = T, showCategory = 15)
