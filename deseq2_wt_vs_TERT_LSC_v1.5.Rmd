...
# Version 1.5: Added significance markers to the TPM graphs
...
p_marker <- ggplot(find_set_LSC, aes(x = gene, y = log10(TPM+1))) +
  geom_bar(
    aes(fill = condition),
...
p_cell <- ggplot(find_set_cell, aes(x = gene, y = log10(TPM+1))) +
  geom_bar(
    aes(fill = condition),
    stat = "identity" , position = position_dodge(0.8),
    width = 0.7) + geom_signif(y_position = c(2.1,1.6, 3.3), xmin = c(0.6,6.6,8.6), 
                            xmax = c(1.4,7.4, 9.4), annotation = c("*","*","*"),
                            tip_length = 0.02)
...
pdf(paste(resultsdir,'LSC_cellcycle.pdf',sep="/") ,width=6,height=7,paper='special')
draw(HTcell)
p_cell
dev.off()
#-------------------------------------------------------------------------------------------
...
#This is a function to generate a PCA plot of your samples. The labels on the axes have to be amnually adjusted based on the result of summary(pc) later on.
PCA <- plotPCA(vst_dds, intgroup = c("conditions","experiment"), returnData = TRUE)
PCA <- ggplot(PCA, aes(PC1, PC2, color = conditions, label = experiment)) +geom_point(size=3) + geom_label_repel(box.padding=0.5) + xlab("PC1 (67.19%)") + ylab("PC2 (19.76%)")
...
#Set a second label stating if the gene is upregulated (1) or downregulated (0)
...
#Create a dataframe stating how many genes are up and down for each gene type
...
p_epi1 <-ggplot(df_epithelial, aes(x = gene, y = log10(TPM+1))) +
  geom_bar(
    aes(fill = condition),
    stat = "identity" , position = position_dodge(0.8),
    width = 0.7) + ggtitle("Epithelial markers")+ geom_signif(y_position = c(1.1,3.1, 2.6,2.0,2.0,2.3,2.5,3.3,4.5,3.0,4.4,1.7), xmin = c(0.6,1.6,2.6,3.6,5.6,7.6,8.6,9.6,10.6,11.6,12.6,13.6), 
                                                              xmax = c(1.4,2.4, 3.4,4.4,6.4,8.4,9.4,10.4,11.4,12.4,13.4,14.4), annotation = c("*","*","*","*","*","*","*","*","*","*","*","*"),
                                                              tip_length = 0.02)
...
p_epi2 <- ggplot(df_epithelial_2, aes(x = gene, y = log10(TPM+1))) +
  geom_bar(
    aes(fill = condition),
    stat = "identity" , position = position_dodge(0.8),
    width = 0.7) + ggtitle("Epithelial markers") + geom_signif(y_position = c(0.7,0.9, 1.8, 0.8, 2.4, 1.5, 2.3), xmin = c(0.6,2.6,3.6,5.6,7.6,9.6,10.6), 
                                                               xmax = c(1.4,3.4, 4.4,6.4,8.4,10.4,11.4), annotation = c("*","*","*","*","*","*","*"),
                                                               tip_length = 0.02)
...
p_epi3 <- ggplot(df_epithelial_3, aes(x = gene, y = log10(TPM+1))) +
  geom_bar(
    aes(fill = condition),
    stat = "identity" , position = position_dodge(0.8),
    width = 0.7) + ggtitle("Epithelial markers")+ geom_signif(y_position = c(2.1,2.0,1.9,0.9,2.0,2.6,2.4), xmin = c(1.6,3.6,4.6,9.6,10.6,11.6,12.6), 
                                                              xmax = c(2.4,4.4, 5.4,10.4,11.4,12.4,13.4), annotation = c("*","*","*","*","*","*","*"),
                                                              tip_length = 0.02)
...
#Find the genes associated with the GO term "intrinsic apoptotic signaling pathway in response to DNA damage by p53 class mediator" that are differentially spliced and differentially expressed.
...
#Find the genes associated with the GO terms "Wnt signaling pathway" and "cell-cell signaling by wnt" that are differentially spliced and differentially expressed.
wnt_genes <- go_genes_diff[c("Wnt signaling pathway","cell-cell signaling by wnt"),]
wnt_genes1 <- wnt_genes[[1]]
wnt_genes2 <- wnt_genes[[2]]
wnt_genes <- c(wnt_genes1,wnt_genes2)
wnt_genes <- unique(wnt_genes)
wnt_genes <- gsub(" ", "", wnt_genes) 
write.table(wnt_genes, file = paste0(resultsdir,'/wnt_genes.tsv'), sep= "\t", row.names=TRUE, col.names =TRUE)

#Then, map these genes to the Wnt signalling KEGG pathway.
...
#Find the genes associated with the GO term "positive regulation of gene expression, epigenetic" that are differentially spliced and differentially expressed.
