...
#Version 1.1: removed SGSeq and de novo alignment section, added annotation
...
#maximum number of counts: is higher than gene counts due to BAM processing steps
max(as.vector(unlist(dxr2$counts_LSC_wt_1)))

#I now want to filter for genes with a total exon count of 100 or higher. Therefore, I first need to obtain the counts
counts_LSC_wt_1 <- as.vector(unlist(dxr2$counts_LSC_wt_1))
counts_LSC_wt_2 <- as.vector(unlist(dxr2$counts_LSC_wt_2))
counts_LSC_TERT_1 <- as.vector(unlist(dxr2$counts_LSC_TERT_1))
counts_LSC_TERT_2 <- as.vector(unlist(dxr2$counts_LSC_TERT_2))

#Then, I calculate the sum of the counts for each exon.
counts <- cbind(counts_LSC_wt_1,counts_LSC_wt_2,counts_LSC_TERT_1,counts_LSC_TERT_2)
counts <- as.data.frame(counts)
counts$sum_counts <- rowSums(counts[,1:4])
sum_counts <- counts$sum_counts
#And I add the sum of the counts for each exon back into the original results data frame
dxr2 <- cbind(dxr2, sum_counts)

#Filter exons based on sum of counts above 100
dxr2_filter <- dxr2[dxr2$sum_counts >=100,]

#Getting the absolute log2FC
dxr2_filter$log2fold_TERT_primary <- as.numeric(dxr2_filter$log2fold_TERT_primary)
dxr2_filter$abslog2 <- abs(dxr2_filter$log2fold_TERT_primary)

#I now add up all absolute log2 fold changes over the exons of a gene
genes <- as.data.frame(dxr2_filter$groupID)
genes <- t(genes)
#I generate a table of all genes and their identifiers
genes <- cbind(genes, dxr2_filter$hgnc_symbol)
genes <- as.data.frame(genes)
colnames(genes) <- c("groupID","hgnc_symbol")

#At the same time, I add up all absolute log2FC per group ID
genes_log2 <- aggregate(abslog2 ~ as.vector(unlist(groupID)), data=dxr2_filter, sum)
...
write.table(genes_log2, file = paste0(resultsdir,'/diff_splicing_genes.tsv'), sep= "\t", row.names=FALSE, col.names =TRUE)
