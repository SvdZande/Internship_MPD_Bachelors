...
#Assigning a label containing the gene name to the top 10 genes with the lowest padj
joined_results <- joined_results[order(joined_results$padj),]
joined_results$label[c(1:10)] <- joined_results$gene.name[c(1:10)]

#Generate a volcano plot indicating significant genes
volcano <-ggplot(joined_results, aes(x=log2FoldChange, y=-log10(padj), col=diffexpressed, label=label)) + geom_point() + ggtitle("Volcano plot of hTERT cells compared to primary cells of all cell types") + geom_vline(xintercept=c(-1, 1), col="black") +
...
#I annotate the 15 genes with the highest log2FCs
log_genes <- result_wt_vs_TERT_logfilter[order(-result_wt_vs_TERT_logfilter$abslog2FoldChange),]
log_genes <- log_genes[1:17,]
log_genes <- rownames(log_genes)

#I now make the labels of the randomly chosen genes
log_rows <- which(rownames(z_logfilter)%in% log_genes, arr.ind = TRUE)
anno_logfilter = anno_mark(at = log_rows, labels = log_genes , which = "row")

#Define the colors of the heatmap
f4 = colorRamp2(c(-1.5, 0, 1.5), c("blue", "white", "red"))
#Make the heatmap using the previously specified colors and labels. The order of the genes in the heatmap  is not correct yet.
htlog <- Heatmap(z_logfilter, name="z-score", column_title = "Heatmap of differentially expressed genes", col = f4,  row_names_gp = gpar(fontsize = 5)) + rowAnnotation(mark= anno_logfilter)
#Because of the incorrect order, we have to reorder z_logfilter based on the order of rows in the heatmap
row_order(htlog)
z_logfilter <- z_logfilter[row_order(htlog),]
#Then, we can find back the genes in the correct order, make the labels, and specify the heatmap
log_genes <- log_genes[order(match(log_genes,rownames(z_logfilter)))]
log_rows <- which(rownames(z_logfilter)%in% log_genes, arr.ind = TRUE)
anno_logfilter = anno_mark(at = log_rows, labels = log_genes , which = "row")
htlog <- Heatmap(z_logfilter, name="z-score", column_title = "Heatmap of differentially expressed genes", col = f4,  row_names_gp = gpar(fontsize = 5)) + rowAnnotation(mark= anno_logfilter)
htlog
...
htlog_cluster <- Heatmap(z_logfilter, km=2,  name="z-score", column_title = "Heatmap of differential  genes", col = f4,  row_names_gp = gpar(fontsize = 5)) + rowAnnotation(mark=anno_logfilter)
...
htlog_primary <- Heatmap(z_logfilter_primary, name="z-score", column_title = "Heatmap of TERT-low differentially expressed genes", col = f4,  row_names_gp = gpar(fontsize = 5)) + rowAnnotation(mark= anno_logfilter_primary)
...
htlog_TERT <- Heatmap(z_logfilter_TERT, km=3, name="z-score", column_title = "Heatmap of TERT-high differentially expressed genes", col = f4,  row_names_gp = gpar(fontsize = 5)) + rowAnnotation(mark= anno_logfilter_TERT)
...
htlog_TERT_subset <- Heatmap(z_logfilter_TERT_subset, km=3, name="z-score", column_title = "Heatmap of TERT-high differentially expressed genes", col = f4,  row_names_gp = gpar(fontsize = 5)) + rowAnnotation(mark= anno_logfilter_TERT_sub)

##Find specified genes of interest in your heatmap
find_all <- c("EHF","PAX6" ,"TP63", "MEIS1", "FOXC1", "OTX1","KRT5","KRT14" , "MCM3", "IVL", "LORICRIN", "FLG", "TGM1")
...
cellcycle <- c("PCNA", "TOP2A", "MCM6", "MKI67", "CCND1", "CDK2", "GMNN", "AURKA", "AURKB", "TERT")
find_set_cell <- NULL
...
p_cell <- ggplot(find_set_cell, aes(x = gene, y = log10(TPM+1))) +
  geom_bar(
    aes(fill = condition),
    stat = "identity" , position = position_dodge(0.8),
    width = 0.7) + geom_signif(y_position = c(2.1, 1.7, 1.9, 3.4), xmin = c(0.6,4.6,8.6,9.6), 
                               xmax = c(1.4, 5.4, 9.4, 10.4), annotation = c("*","*", "*", "*"),
                               tip_length = 0.02)
...
barplot(ego, showCategory=15, order = T, title = "Barplot for padj genes")

barplot(ego_2, showCategory=15, order = T, title = "Barplot for log2 genes") 
...
barplot(ego_new, showCategory=15, order = T, title = "Simplified barplot for padj genes")

barplot(ego_new_2, showCategory=15, order = T,title = "Simplified barplot for log2 genes")
...
high_log2_GO <- barplot(ego_high_log, order = T, showCategory = 15, title = "Differentially upregulated genes in TERT")
low_log2_GO <- barplot(ego_low_log, order = T, showCategory = 15, title = "Differentially downregulated genes in TERT")
high_log2_GO
low_log2_GO
...
plotPCA.dim <- function (object, intgroup = "conditions", ntop = 500, returnData = FALSE) 
{
  ...
  if (returnData) {
   ...
   ggplot(d, aes(PC1, PC2, color = conditions, label = name)) +geom_point(size=3) + geom_label_repel(size=10,box.padding=0.5) + xlab(paste0("PC1: ",round(var_explained_df[1,2]*100,digits = 2),"%")) + ylab(paste0("PC2: ",round(var_explained_df[2,2]*100, digits = 2),"%"))
...
PCA_pos_ALL <- barplot(ego_PC1_pos, order = T, showCategory = 15, title = "Processes giving positive weight to PC1")
PCA_neg_ALL <- barplot(ego_PC1_neg, order = T, showCategory = 15, title = "Processes giving negative weight to PC1")
PCA_pos_ALL
PCA_neg_ALL
...
barplot(ego_PC2_pos, showCategory = 15, order = T, title = "GO analysis of genes giving positive weight to PC2")
barplot(ego_PC2_neg, showCategory = 15, order = T, title = "GO analysis of genes giving negative weight to PC2")
...
log2_KC_genes <- read.table("/scratch/szande_jarts/diff_gene_expression_wt_vs_TERT/20220629/wt_vs_TERT_KC_genes_2_v3.tsv")
log2_KC_genes <- as.vector(log2_KC_genes[,1])
log2_LSC_genes <- read.table("/scratch/szande_jarts/diff_gene_expression_wt_vs_TERT/20220629/wt_vs_TERT_LSC_genes_2.v2.tsv")
...
DESeq_KC <- read.csv("/scratch/szande_jarts/diff_gene_expression_wt_vs_TERT/20220629/wt_vs_TERT_KC_DESeq2_v3.tsv", header = TRUE, sep= "\t")
DESeq_KC$Gene.name <- rownames(DESeq_KC)
#Then, the DESeq2 result of the LSC analysis
DESeq_LSC <- read.csv("/scratch/szande_jarts/diff_gene_expression_wt_vs_TERT/20220629/wt_vs_TERT_LSC_DESeq2.v2.tsv", header = TRUE, sep= "\t")
...
#LSC
diff_LSC <- read.csv("/scratch/szande_jarts/diff_gene_expression_wt_vs_TERT/20220629/wt_vs_TERT_LSC_genes_2.v2.tsv", header = FALSE, sep= "\t")
colnames(diff_LSC) <- c("Gene.name")
diff_LSC$Cell.type <- "LSC"

#KC
diff_KC <- read.csv("/scratch/szande_jarts/diff_gene_expression_wt_vs_TERT/20220629/wt_vs_TERT_KC_genes_2_v3.tsv", header = FALSE, sep= "\t")
